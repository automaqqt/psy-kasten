...
// ── IGT ────────────────────────────────────────────────
export function exportIgtCSV(data) {
  let trials = data.trials && Array.isArray(data.trials) ? data.trials : reconstructArray(data);

  const headers = ['Trial Number', 'Deck', 'Reward', 'Loss', 'Net', 'Balance After', 'Reaction Time (ms)'];
  const rows = trials.map(trial => [
    trial.trialNumber,
    trial.deck ?? '',
    trial.reward ?? '',
  ...
  const csv = buildCSV(headers, rows);
  downloadCSV(csv, `igt-results-${new Date().toISOString().split('T')[0]}.csv`);
}


// ── IAT ────────────────────────────────────────────────
export function exportIatCSV(data) {
  let trials = data.trials && Array.isArray(data.trials) ? data.trials : reconstructArray(data);

  const headers = [
    'Block', 'Kind', 'Compat', 'Practice', 'TrialInBlock',
    'Word', 'Category', 'CorrectSide', 'ResponseSide',
    'ReactionTime_ms', 'Correct'
  ];
  const rows = trials.map(tr => [
    tr.blockIndex + 1, tr.kind || '',
    tr.compat === null || tr.compat === undefined ? '' : (tr.compat ? 1 : 0),
    tr.practice ? 1 : 0, tr.trialInBlock,
    tr.word, tr.category,
    tr.correctSide, tr.responseSide,
    Math.round(tr.reactionTime || 0),
    tr.correct ? 1 : 0,
  ]);

  rows.push([]);
  rows.push(['--- Summary ---']);
  if (typeof data.dScore === 'number') rows.push(['D-score', data.dScore.toFixed(3)]);
  if (typeof data.iatEffect === 'number') rows.push(['IAT effect (ms)', Math.round(data.iatEffect)]);
  if (typeof data.meanRtCompat === 'number') rows.push(['Mean RT compatible (ms)', Math.round(data.meanRtCompat)]);
  if (typeof data.meanRtIncompat === 'number') rows.push(['Mean RT incompatible (ms)', Math.round(data.meanRtIncompat)]);
  if (typeof data.accuracy === 'number') rows.push(['Accuracy (%)', data.accuracy.toFixed(1)]);

  const csv = buildCSV(headers, rows);
  downloadCSV(csv, `iat-results-${new Date().toISOString().split('T')[0]}.csv`);
}


// ── EFT ────────────────────────────────────────────────
export function exportEftCSV(data) {
  let trials = data.trials && Array.isArray(data.trials)
    ? data.trials
    : reconstructArray(data.trials || data);

  const headers = [
    'Trial Number', 'Condition', 'Target', 'Flanker',
    'Expected Response', 'Given Response', 'Correct',
    'Reaction Time (ms)', 'Timed Out'
  ];
  const rows = trials.map(tr => [
    tr.trialNumber,
    tr.condition || '',
    ...
    tr.correct || '',
    tr.response || '',
    tr.correctResponse ? 1 : 0,
    tr.reactionTime != null ? Math.round(tr.reactionTime) : '',
    tr.timedOut ? 1 : 0,
  ]);

  if (typeof data.meanRT === 'number' || typeof data.flankerEffect === 'number') {
    rows.push([]);
    rows.push(['--- Summary ---']);
    if (typeof data.overallAccuracy === 'number') rows.push(['Overall accuracy', (data.overallAccuracy * 100).toFixed(1) + '%']);
    if (typeof data.meanRT === 'number') rows.push(['Mean RT (ms)', Math.round(data.meanRT)]);
    if (typeof data.meanRTCongruent === 'number') rows.push(['Mean RT congruent (ms)', Math.round(data.meanRTCongruent)]);
    if (typeof data.meanRTIncongruent === 'number') rows.push(['Mean RT incongruent (ms)', Math.round(data.meanRTIncongruent)]);
    if (typeof data.meanRTNeutral === 'number') rows.push(['Mean RT neutral (ms)', Math.round(data.meanRTNeutral)]);
    if (typeof data.flankerEffect === 'number') rows.push(['Flanker effect (ms)', Math.round(data.flankerEffect)]);
  }

  const csv = buildCSV(headers, rows);
  downloadCSV(csv, `eft-results-${new Date().toISOString().split('T')[0]}.csv`);
}


// ── PCP ────────────────────────────────────────────────
// TODO: Update CSV columns and data mapping for your test
export function exportPcpCSV(data) {
  let trials = data.trials && Array.isArray(data.trials) ? data.trials : reconstructArray(data);
  ...
  const csv = buildCSV(headers, rows);
  downloadCSV(csv, `pcp-results-${new Date().toISOString().split('T')[0]}.csv`);
}
