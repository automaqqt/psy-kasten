...
/**
 * Validate IGT (Iowa Gambling Task) test results
 */
function validateIgtData(data) {
  const errors = [];

  if (!isArray(data.trials)) {
    errors.push('trials must be an array');
  } else {
    const validDecks = ['A', 'B', 'C', 'D'];
    data.trials.forEach((trial, index) => {
      if (!isObject(trial)) {
        errors.push(`trials[${index}] must be an object`);
        return;
      ...
      if (!isString(trial.deck) || !validDecks.includes(trial.deck)) {
        errors.push(`trials[${index}].deck must be one of A, B, C, D`);
      }
      if (typeof trial.reward !== 'number') {
        errors.push(`trials[${index}].reward must be a number`);
      }
      if (typeof trial.loss !== 'number') {
        errors.push(`trials[${index}].loss must be a number`);
      }
      if (typeof trial.net !== 'number') {
        errors.push(`trials[${index}].net must be a number`);
      }
      if (typeof trial.balanceAfter !== 'number') {
        errors.push(`trials[${index}].balanceAfter must be a number`);
      }
    });
  }
...
/**
 * Validate IAT (Implicit Association Test) results
 * Shape: { trials: [{ blockIndex, kind, compat, practice, trialInBlock, word, category,
 *                     correctSide, responseSide, correct, reactionTime }],
 *          iatEffect, dScore, meanRtCompat, meanRtIncompat, accuracy, blockStats, settingsUsed }
 */
function validateIatData(data) {
  const errors = [];
  ...
  if (!isArray(data.trials)) {
    ...
    data.trials.forEach((trial, index) => {
      ...
      if (typeof trial.blockIndex !== 'number') {
        errors.push(`trials[${index}].blockIndex must be a number`);
      }
      if (!isString(trial.kind)) {
        errors.push(`trials[${index}].kind must be a string`);
      }
      if (!isString(trial.word)) {
        errors.push(`trials[${index}].word must be a string`);
      }
      if (!isBoolean(trial.correct)) {
        errors.push(`trials[${index}].correct must be a boolean`);
      }
      if (!isPositiveNumber(trial.reactionTime)) {
        errors.push(`trials[${index}].reactionTime must be a non-negative number`);
      }
    });
  }
...
/**
 * Validate Eriksen Flanker Task (EFT) results
 * Expected shape: { trials: [{ trialNumber, condition, target, flanker, correct, response, correctResponse, reactionTime, timedOut }], ... }
 */
function validateEftData(data) {
  const errors = [];
  const validConditions = ['congruent', 'incongruent', 'neutral'];
  const validResponses = ['left', 'right'];
  ...
  if (!isArray(data.trials)) {
    ...
    data.trials.forEach((trial, index) => {
      ...
      if (!isString(trial.condition) || !validConditions.includes(trial.condition)) {
        errors.push(`trials[${index}].condition must be one of ${validConditions.join(', ')}`);
      }
      if (!isString(trial.target)) {
        errors.push(`trials[${index}].target must be a string`);
      }
      if (!isString(trial.flanker)) {
        errors.push(`trials[${index}].flanker must be a string`);
      }
      if (!isString(trial.correct) || !validResponses.includes(trial.correct)) {
        errors.push(`trials[${index}].correct must be 'left' or 'right'`);
      }
      if (trial.response !== null && (!isString(trial.response) || !validResponses.includes(trial.response))) {
        errors.push(`trials[${index}].response must be 'left', 'right', or null`);
      }
      if (!isBoolean(trial.correctResponse)) {
        errors.push(`trials[${index}].correctResponse must be a boolean`);
      }
      if (trial.reactionTime !== null && !isPositiveNumber(trial.reactionTime)) {
        errors.push(`trials[${index}].reactionTime must be a positive number or null`);
      }
      if (!isBoolean(trial.timedOut)) {
        errors.push(`trials[${index}].timedOut must be a boolean`);
      }
    });
  }
