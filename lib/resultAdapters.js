 ...
 * Stored: { trials: [...], summary: { meanAccuracy, totalCorrect, ... } }
 * Component: MotResults({ trials, settings, t })
 */
export function adaptMotResult(data) {
  if (!data) return null;
  ...
  return { trials, settings: data.settings || null };
}


/**
 * Prlt Test
 * Stored: { trials, reversalsCompleted, accuracy, winStayRate, loseShiftRate, settingsUsed }
 * Component: PrltResults({ trials, reversalsCompleted, settings, t })
 */
export function adaptPrltResult(data) {
  if (!data) return null;
  ...
  return {
    trials,
    reversalsCompleted: data.reversalsCompleted || 0,
    settings: data.settingsUsed || null,
  };
}
...
/**
 * Nback Test
 * Stored: { trials, nLevel, accuracy, meanRT, hits, misses, falseAlarms, correctRejections, dPrime, totalTrials }
 * Component: NbackResults({ trials, nLevel, t })
 */
export function adaptNbackResult(data) {
  if (!data) return null;
  ...
  return { trials, nLevel: data.nLevel || 2 };
}


/**
 * PAL (Paired Associates Learning)
 * Stored: { responses, firstAttemptMemoryScore, totalErrorsAdjusted, levelsCompleted, settings }
 * Component: PalResults({ responses, levels, maxAttempts, t })
 */
export function adaptPalResult(data) {
  if (!data) return null;
  ...
  if (data.responses && Array.isArray(data.responses)) {
    ...
    responses = reconstructArray(data);
  }

  if (!responses || responses.length === 0) return null;

  const levels = data.settings?.levels || [2, 4, 6, 8];
  const maxAttempts = data.settings?.maxAttempts || 4;
  return { responses, levels, maxAttempts };
}


/**
 * Sternberg Working Memory Task
 * Stored: array of trial objects
 * Component: StbResults({ trials, t })
 */
export function adaptStbResult(data) {
  if (!data) return null;
 ...
 * Stored: { partA: { timeSec, errors, aborted, totalCircles, clicks }, partB: {...}, bMinusA }
 * Component: TmtResults({ segmentResults, t })
 */
export function adaptTmtResult(data) {
  if (!data) return null;
  ...
  if (data.partA) {
    segmentResults.push({
      ...
      part: 'A',
      practice: false,
      timeSec: data.partA.timeSec,
      errors: data.partA.errors || 0,
      aborted: !!data.partA.aborted,
      totalCircles: data.partA.totalCircles || 25,
      clicks: Array.isArray(data.partA.clicks) ? data.partA.clicks : reconstructArray(data.partA.clicks),
    });
  }
  if (data.partB) {
    segmentResults.push({
      ...
      part: 'B',
      practice: false,
      timeSec: data.partB.timeSec,
      errors: data.partB.errors || 0,
      aborted: !!data.partB.aborted,
      totalCircles: data.partB.totalCircles || 25,
      clicks: Array.isArray(data.partB.clicks) ? data.partB.clicks : reconstructArray(data.partB.clicks),
    });
  }
 ...
 *   summary: { adjBlue, adjYellow, adjOrange, explosions, totalBalloons, totalEarnedCents }
 * }
 * Component: BartResults({ trials, totalEarnedCents, t })
 */
export function adaptBartResult(data) {
  if (!data) return null;
  ...
  const totalEarnedCents = typeof data.totalEarnedCents === 'number'
    ? data.totalEarnedCents
    : trials.reduce((sum, t) => sum + (t.exploded ? 0 : (t.earningsCents || 0)), 0);

  return { trials, totalEarnedCents };
}


/**
 * Iowa Gambling Task
 * Stored: { trials, totalTrials, startingBalance, finalBalance, deckCounts, goodChoices, badChoices, netScore }
 * Component: IgtResults({ trials, startingBalance, finalBalance, t })
 */
export function adaptIgtResult(data) {
  if (!data) return null;
  ...
  const startingBalance = data.startingBalance ?? 2000;
  const finalBalance = data.finalBalance ?? trials[trials.length - 1]?.balanceAfter ?? startingBalance;
  return { trials, startingBalance, finalBalance };
}


/**
 * IAT (Implicit Association Test)
 * Stored: { trials, iatEffect, dScore, meanRtCompat, meanRtIncompat, accuracy, blockStats, settingsUsed }
 * Component: IatResults({ trials, onRestart, t })
 */
export function adaptIatResult(data) {
  if (!data) return null;
  ...
  return { trials, onRestart: null };
}


/**
 * Eriksen Flanker Task (EFT)
 * Stored: { trials, totalTrials, overallAccuracy, meanRT, meanRTCongruent, meanRTIncongruent, meanRTNeutral, flankerEffect, ... }
 * Component: EftResults({ trials, t })
 */
export function adaptEftResult(data) {
  if (!data) return null;

  let trials;
  if (data.trials && Array.isArray(data.trials)) {
    trials = data.trials;
  } else if (data.trials && typeof data.trials === 'object') {
    trials = reconstructArray(data.trials);
  } else {
    trials = reconstructArray(data);
  }
 ...
 * Stored: { trials, ... }
 * Component: PcpResults({ trials, falseStarts, t })
 */
export function adaptPcpResult(data) {
  if (!data) return null;
...
const ADAPTERS = {
  ...
  'mot': adaptMotResult,
  'prlt': adaptPrltResult,
  'nback': adaptNbackResult,
  'pal': adaptPalResult,
  'stb': adaptStbResult,
  'tmt': adaptTmtResult,
  'bart': adaptBartResult,
  'igt': adaptIgtResult,
  'iat': adaptIatResult,
  'eft': adaptEftResult,
  'pcp': adaptPcpResult,
