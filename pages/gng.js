...
export async function getStaticProps({ locale }) {
  return {
    props: {
      ...(await serverSideTranslations(locale, ['common', 'gng'])),
    },
  };
}

export default function GNGPage(props) {
  const router = useRouter();
  const { t } = useTranslation(['gng', 'common']);
  ...
  // Submission handler - ONLY called by GNGTest if NOT standalone
